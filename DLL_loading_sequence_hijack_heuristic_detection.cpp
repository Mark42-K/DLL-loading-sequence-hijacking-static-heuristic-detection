...
DWORD RVA_To_FOA(PIMAGE_NT_HEADERS PImage_NT_Header, DWORD Virtual_Address);
...
{
	DWORD temp;
	for (DWORD i = 0; i < NumberOfMember; i++)
	{
		DWORD temp_member = (*Array)[i];
		int temp_index = i;
		for (DWORD j = i + 1; j < NumberOfMember; j++)
		{
				 ...
				 DWORD Virtual_Address)
{
	...
	//Convert a visual address to a file offset address
	for (number; number < Number_Of_Section; number++)
	{
		PIMAGE_SECTION_HEADER P = (PIMAGE_SECTION_HEADER)((DWORD)PImage_Section_Header + Image_Section_Header_Size * number);
		if ((P->VirtualAddress <= Virtual_Address) && (P->VirtualAddress + P->Misc.VirtualSize >= Virtual_Address))
		{
			FOA = Virtual_Address - (P->VirtualAddress - P->PointerToRawData);
			break;
		}
	}
	
	//Detemine whether the visual address if out of range
	if (number >= Number_Of_Section)
	{
...
/*bool Verify_Digital_Signature(string File_Path)
{
	...
	case TRUST_E_NOSIGNATURE:
		if (TRUST_E_NOSIGNATURE == GetLastError() || TRUST_E_SUBJECT_FORM_UNKNOWN == GetLastError() || TRUST_E_PROVIDER_UNKNOWN == GetLastError())
		{
			cout << "The file is not signed" << endl;
		}
		else
		{
			cout << "An unknown error occurred trying to verify the signature of the file" << endl;
		}
		break;
...
int DLLHijack_Detection(char* Image_Address, PIMAGE_NT_HEADERS PImage_NT_Header)
{
	...
	{
		Tread_Number++;
		cout << "Detected Trojan:Reason:Same_Export_Name:\t" << "DLLName: " << DLL_Name  << endl;
	}
	else if (Same_Export_Addr(AddressOfFunction_Duplicate, Number_Of_Functions,DLL_Name))
	{
		Tread_Number++;
		cout << "Detected Trojan:Reason:Same_Export_Addr >= 3\t" << "DLLName: " << DLL_Name << endl;
	}
	else if (Same_Addr_Interval(AddressOfFunction_Duplicate, Number_Of_Functions))
	{
		Tread_Number++;
		cout << "Detected Trojan:Reason:Same_Addr_Interval >= 3\t" << "DLLName: " << DLL_Name << endl;
	}

	delete[] AddressOfFunction_Duplicate;
	return 0;
}
...
int main(int argc, char* argv[])
{
	if (argc != 2)
	{
		cout << "Method of use:" << endl;
		cout << "\tThis program path + the file path that needs to be scanned" << endl;
		return 0;
	}
	...
	for (HANDLE HSearch = FindFirstFileA(File_Path.c_str(), &File_Data); stage; stage = FindNextFileA(HSearch, &File_Data))
	{
		...
		else if (!(HFMO = CreateFileMappingA(HFile, NULL, PAGE_READONLY, 0, 0, NULL)))
		{
			CloseHandle(HFile);
			continue;
		}
		else if (!(Image_Address = (char*)MapViewOfFile(HFMO, FILE_MAP_READ, 0, 0, 0)))
		{
			CloseHandle(HFMO);
			CloseHandle(HFile);
			continue;
		...
		else if (PImage_NT_Header->OptionalHeader.DataDirectory[0].VirtualAddress == 0)
		{
			UnmapViewOfFile(Image_Address);
			CloseHandle(HFMO);
			CloseHandle(HFile);
